...
**Terza passata, da altre due stagioni.** Due log senza commento: 14 vittorie su 15 gare, tutte sulla Drift tranne una. La tassa sulla scivolata in velocità aveva fatto il suo: a Harbour il distacco in qualifica era sceso dal 10% al 3%, a Crown 2%, a Kettle la Drift era più lenta (P9 in qualifica, P6 in gara). Ma sui circuiti con i tornanti — Comb, Anchor, Thunder, Serpent — i distacchi erano ancora dell'8-12%. Lo stesso giorno, la sua Medium a Crown faceva lo stesso tempo della Drift: quindi la Drift non era più un superpotere sui filanti, ed era ancora la gomma giusta ovunque ci fosse un tornante.

Il perché l'ha detto uno strumento nuovo: il pilota del gioco guidato **sparato a tutte le velocità** e col gas piantato nei lenti — cioè come guida Nicola secondo la sua telemetria — contro la Medium, da solo, profilo e telaio inchiodati. Con la fisica della seconda passata: **Comb −6.8%, Kart −4.3%, Thunder −2.7%, Kettle −4.4%**. E la tassa, per quanto alta (provata fino a 3.0, e dai 100 px/s, e dagli 80), non la spostava: il guadagno non era la scivolata. Era `hook`, il bonus di sterzata nei lenti messo nella *prima* stesura — quando la gomma non scivolava affatto e le serviva qualcosa per essere veloce da qualche parte. A 0.58 sotto i 171 px/s la Drift **sterzava più della Medium col solo volante**, e adesso ci si aggiungeva la coda che la fa ruotare.

Quindi: `hook` da 0.58 a **0.30** — il volante è più lento di una Medium ovunque, e quello che la gomma ha in un tornante ce l'ha dal gas. E la tassa della scivolata si paga **sul sovrasterzo in più rispetto a una slick** allo stesso carico (`_osSlick`), da 0 px/s: una slick non paga niente nemmeno nei tornanti, la Drift paga esattamente quello che ha di più. Stessa misura dopo: Comb −1.2%, Kart −1.1%, Thunder +0.8%, Anchor +4.7%, Serpent +0.8%, Harbour +2.8%, Kettle −0.5%, Oval −1.7% — la forma di una specialista, alla taglia di una specialista. L'IA, per starle dietro, la guida sparata anche nei lenti (`AI.driftSend` 0.20 da 0 px/s): con la griglia piena è pari su Comb, Kart e Thunder e 1-5% indietro su Harbour, F1 e Oval, zero ritiri.

Il test a sterzo e gas piantati per 1.2 s, adesso: a 240 px/s la Drift deriva di 46° e finisce a **111 px/s** (la Hard: 12°, 259); a 120 di 54° e finisce a 91. Tenere la scivolata costa, e costa tanto; la coda è lì a qualunque velocità, e la velocità la fa chi la dosa.

**E poi c'è un'altra cosa, che la gomma non c'entra.** Nei cinque log precedenti a tutto questo — quattro stagioni e mezza, quasi sempre sulla Medium, IA a *impossible* — Nicola ha vinto il **55-60% delle gare**, con tempi in qualifica del 5-10% più bassi dell'IA su metà dei circuiti. Il livello che si chiama impossibile non lo è per lui. La Drift ha trasformato un 60% in un 90%; rimessa al suo posto, il 60% resta. Se vuole qualcuno contro cui correre davvero, il posto dove guardare è il tetto dell'IA, non la gomma.

### 2.4quinquies-et-vicies Il tetto dell'IA: la traiettoria (Apex 3)

«Ricalibra i livelli dell'IA. *Alien* mantienilo com'è ora, cioè il livello più difficile ma senza il "bonus" dei danni. Aggiungi un livello oppure alza *impossible* (e aggiusta di conseguenza anche gli altri), come preferisci.»

Il punto di partenza era la chiusura della sezione precedente: sulla Medium, contro l'IA a *impossible*, Nicola vince il 55-60% delle gare e in qualifica è più veloce dell'IA su metà dei circuiti. Rimisurato circuito per circuito dai suoi log (asciutto, Medium, telaio Aero) contro il giro secco dell'IA al tetto: **Rectangle −14%, Zipper −11%, Harbour −11%, Crown −10%, Serpent −9%, Kettle −7%, Crossover −7%**, F1 −5%; pari a Peanut e Boomerang; più lento solo all'Oval (+4%) e a Circle (+9%). La forma dice tutto: dove la pista è rette e curve lui vola, dove è un curvone solo l'IA è avanti.

**Scelta: alzare *impossible*, non aggiungere un livello.** Un livello in più sopra un tetto che lui batte sei volte su dieci sarebbe stato un nome nuovo per lo stesso problema. *Alien* resta per costruzione quello che era — la stessa andatura di *impossible*, nessuna vettura debole in griglia, nessuno sconto sui danni — e quindi si alza con lui.

**Il tetto non era il tetto.** L'intestazione di `ai.js` diceva che *impossible* «è già al limite della macchina, non del proprio coraggio»: le scansioni di `cornerFactor`, `brakeConfidence`, `provoke`, `steerTau`, `lookBase` l'hanno confermato di nuovo (nessuna sposta il giro di più dell'1%). Ma la traccia di velocità sul Rectangle ha detto un'altra cosa: l'IA **frenava a 167 px/s quattro volte al giro**, perché la traiettoria che le veniva data — il rilassamento laplaciano del centro pista, fermato a 600 passaggi — ha un gomito di raggio ~100 px all'ingresso e all'uscita di ogni curva. La strada permette un arco di raggio ~190 preso a 220+ px/s, ed è quello che guida Nicola: la sua telemetria a Rectangle dice *0% del giro sotto i 160 px/s, media 266*, contro una media di 224 dell'IA su una linea più corta della sua. Il limite era la linea, non la macchina.

**La linea ora si ottimizza, in due passi (`track.js`, "THE LINE, AND HOW IT IS CHOSEN").**

1. *Ricerca diretta sugli offset laterali.* Gobbe a coseno rialzato di 2-24 nodi spinte a destra e a sinistra, una mossa tenuta se scende un **tempo sul giro analitico** (`_lineModel`): un profilo di velocità sulla candidata con il limite di sterzata (`v = ωR/(1+ωR/500)`, la forma chiusa della fisica), il **limite di potenza** — in curva la velocità è in ritardo sulla prua di un angolo di deriva β con sin β = v/(3.5R), e la forza laterale v²/R che tiene la curva ha una componente v³/(3.5R²) contro il moto: a 200 px/s su R=150 sono 100 px/s², tutto il motore, quindi una curva di raggio medio è limitata dalla potenza quanto dallo sterzo e una linea che non lo sa sceglie raggi troppo stretti (è l'errore che aveva la prima versione dell'ottimizzatore all'Oval) — e il **cambio di deriva nelle S**: nel passare da +β a −β la prua deve ruotare anche di quello, letto su ±6 nodi perché il raggio a tre punti è rumoroso; senza questo termine la prima stesura mandava l'IA fuori strada alla prima chicane di Thunder (+16%). Poi accelerazione e frenata avanti e indietro, e la somma dei ds/v. Il proxy è tarato: 7.8 s contro 8.3 reali all'Oval, 10.35 contro 10.45 a Rectangle.
2. *Un giudice.* Il proxy ordina bene ma non perfettamente, quindi ogni candidata — i tre rilassamenti di prima e le ottimizzate a tre margini dal bordo (20, 17 e 14 px) — viene **guidata dall'IA stessa** per un giro lanciato (`judgeRacingLine` in `main.js`: *impossible*, pilota senza stile, Medium, asciutto, telaio di riferimento) e vince la più veloce. Dove l'ottimizzatore non aiuta vince il rilassamento e non si perde niente.

Il costo (1-3 s per circuito in JavaScript) non si paga in gioco: **`genlines.js`** esegue esattamente questo codice senza testa, con tre giri di giudizio per candidata, e scrive **`lines.js`**, una tabella degli offset vincenti indicizzata per `RACING_LINE_VERSION` e per `geomHash()` del circuito (segmenti, larghezza, verde — la stessa impronta del libro dei record). Un circuito modificato non trova più la sua voce e torna sulla strada lenta (ottimizza, giudica, ricorda in `localStorage` sotto la stessa chiave): la linea giusta arriva comunque, un paio di secondi dopo, una volta. Nel file c'è scritto: **quando cambia un circuito, l'ottimizzatore o la fisica che rispecchia, si alza la versione e si rilancia `genlines.js`.**

La stessa linea serve **tutti i livelli** (i profili deboli la mescolano verso il centro con `lineBlend`, come hanno sempre fatto); la linea 'fast' è lo stesso oggetto. Misurato con l'IA da sola, pilota neutro, Aero, Medium, asciutto, il tetto ha guadagnato in media il **4.5%**: Anchor −11.7%, Harbour −8.8%, Rectangle −8.3%, Serpent −8.2%, Circo Massimo −7%, Triangle e Kart −5.5%, Crown −4.6%, Kettle, F1, Crossover e Peanut −3%, Oval −2.5%, Zipper −2%; Circle e Thunder 0 (a Thunder l'ottimizzatore non batte il rilassamento: la sequenza di S gli resta indigesta, è il punto aperto).

**Le conseguenze in gara, e tre correzioni all'IA.** Una linea vera va al bordo della strada all'uscita di ogni curva, e il profilo di velocità dell'IA era della *linea*: una vettura 30 px larga di dove voleva stare — spinta fuori dal traffico, in difesa, sulle gomme finite — era 30 px sull'erba, dove con la vecchia linea stretta all'interno era strada di riserva. Misurato a *impossible*, 36 gare a dieci vetture: i ritiri erano raddoppiati (da 0.7 a 1.4 a gara) e i danni da muro, prima nulli, erano un terzo del totale.

- **Finire la strada (`ai.js`, blocco 5).** A ogni nodo in avanti l'IA si chiede anche: *se tengo l'errore che ho, ci sto sulla strada?* Dove non ci sta, l'eccedenza va girata via sulla distanza a quel nodo — curvatura 2e/d² in più di quella della linea — e quel percorso più stretto ha la sua velocità di sterzata, che tappa il profilo come una curva qualunque. Niente cambia per una vettura sulla sua linea; una larga rallenta di quello che le serve. I danni da muro sono tornati a zero e il tempo a muro da 0.05% allo 0.02%.
- **Un'auto ferma è un ostacolo.** La legge di inseguimento (velocità dell'altra + distanza × `gapGain`) a 100 px da una vettura ferma diceva 244 px/s, che i freni non trasformano in uno stop in 100 px: erano coppie di ritiri allo stesso secondo nel log, una vettura girata in mezzo alla strada e una che le arrivava addosso a 150-230. Sotto gli 80 px/s, o sotto il 60% della nostra velocità, il tetto diventa la velocità che i freni possono smaltire sulla distanza, e la manovra per aggirarla parte da tutta la finestra e non dal raggio di sorpasso.
- **La colonna di partenza si tiene.** Allo spegnersi delle luci ogni vettura puntava subito la linea ideale: dieci vetture in fila indiana prima della prima curva. Finché dura la cautela di partenza l'offset laterale viene fissato una volta da dove la vettura *è* e tenuto; la legge di inseguimento e la spinta ruota a ruota agiscono comunque. I contatti pesanti nei primi sei secondi sono passati da 13-21 a 4 su 24 gare.

Con le tre correzioni i ritiri a *impossible* sono **~1.1 a gara** nella stessa misura (36 gare, sole IA), contro 0.7 con la vecchia linea: la griglia va più forte e si tocca di più, soprattutto nel primo giro. Per taratura, nei 75 round a *impossible* dei log di Nicola — con lui in pista — i ritiri dell'IA erano già **1.23 a gara**. I tentativi che non hanno spostato il numero e sono stati tolti: cautela di partenza più lunga (7 e 10 s), `gapGain`/`safeGap` di *hard* al tetto, e una mira che sterza la direzione del moto invece della prua (più veloce da sola dell'1.5%, più trottole in gara).

**La scala dopo.** Stessa misura dell'intestazione di `ai.js` (giro secco, pilota neutro, Aero, Medium, asciutto, diciotto circuiti):

| | easy | medium | hard | impossible | alien |
|---|---|---|---|---|---|
| prima | +40.0% | +22.1% | +8.6% | 0 | 0 |
| **ora** | **+41.5%** | **+22.2%** | **+8.0%** | **0** | **0** |

Tutti i livelli condividono la linea, quindi i gradini sono saliti insieme e la scala ha tenuto la forma: i fattori di `AI_PROFILES` **non sono stati ritoccati** — "aggiustare di conseguenza gli altri" è successo da sé. Contro i tempi di Nicola, il nuovo tetto è: pari a Harbour, Serpent, Peanut e Boomerang; più veloce all'Oval (5%) e a Circle (7%); ancora più lento del 3-7% a Rectangle, Zipper, Crown, Crossover e Kettle. Quello che resta è *come* guida una curva lenta, non su che linea: un lavoro da fare sul controllore, non sulla traiettoria, ed è annotato fra i punti aperti.

Il libro dei record si ricostruisce una volta: la versione della linea fa parte dell'impronta della fisica (`exPhysHash`), e l'impronta geometrica dei circuiti è ora `track.geomHash()`, la stessa che indicizza `lines.js`.

Gli strumenti, tutti in `/root/apex`: `genlines.js` (genera `lines.js`), `ladder.js` (la scala, giro secco per livello e circuito), `linepick.js`/`lineopt2.js` (candidate contro giudice), `trace.js`/`linecmp.js` (tracce di velocità e confronto per settore), `trackerr.js` (errore di inseguimento della linea), `racecheck.js`/`contactstats.js`/`wreckwhy.js` (gare di controllo: ritiri, contatti pesanti, gli ultimi secondi prima di un urto).

### 2.4sexies-et-vicies Il pit wall che teneva in ostaggio tutte le sessioni (Apex 3)

«La versione con i pit stop non funziona», con due schermate: la qualifica avviata — HUD giusto, «Qualifying — warm-up lap», 0 km/h, «Dry», il tasto End Qualifying — sopra una **tela nera**, e un riquadro rosso: `Error: Script error. Line: 0. URL: .../main.js`.

**Il blocco.** In `gameLoop` la prima cosa che si guarda è se il pit wall è aperto: se `pitPanelSeat` non è zero la gara è *tenuta*, si chiede il fotogramma successivo e si esce. Il codice che richiude il pannello da solo — pilota uscito, sosta fatta, macchina rotta — vive dentro `updatePhysics`, cioè **dopo quel `return`**, e per di più dentro `if (pitModeOn)`. Quindi: chi apriva il pit wall e poi usciva dalla sessione (il tasto Quit non lo chiudeva) lasciava `pitPanelSeat = 1` per sempre. La sessione dopo — una qualifica, dove `pitModeOn` è falso per definizione — partiva, scriveva tutto il suo HUD, e poi il ciclo tornava indietro a ogni fotogramma senza disegnare **niente**: la tela restava quella grigia del menu e la macchina non si muoveva. Uno stallo che non poteva sciogliersi da solo, perché la cosa che l'avrebbe sciolto era esattamente quella che il `return` impediva di eseguire. Riprodotto in laboratorio (`pitfreeze.js`): apri il pit wall, esci col Quit, avvia una qualifica → **0 fotogrammi disegnati in 0.8 s**; con la correzione, 92.

Tre righe di rimedio, in ordine di importanza:

- **`gameLoop` non si fa più tenere da un pannello senza padrone**: se `pitModeOn` è falso o `pitPanelCar()` è nullo, il pit wall si chiude lì e la gara riparte. Questo da solo rende lo stallo impossibile, qualunque strada lo abbia prodotto.
- **il tasto Quit chiude il pit wall** (e azzera il cronometro della pausa, che altrimenti restituiva alla gara successiva tutto il tempo passato nei menù).
- **`startQualifying` lo azzera come già faceva `startGame`**: nessuna sessione eredita un pit wall aperto.

Non c'entrava la traiettoria nuova: è un difetto che la build con i pit stop si portava dietro da quando il pannello esiste, e che si vede solo se si esce di sessione col pannello aperto.

**E il riquadro rosso non diceva niente, il che è un secondo difetto.** Il gioco si apre da `file://`, dove ogni file è un'origine a sé: per il browser uno script caricato da un altro file è *cross-origin*, e `window.onerror` riceve per specifica `"Script error."`, riga 0 e nient'altro. Safari fa esattamente così. Un errore che però **catturiamo noi** non viene sanificato: quindi i due posti da cui si entra nel codice del gioco — un fotogramma di animazione e un gestore di eventi del DOM — ora sono avvolti in un `try/catch` che stampa messaggio e prime righe dello stack in un pannello leggibile (`racing.html`, "THE ERROR PANEL"). Tre dettagli che contano: l'errore viene **rilanciato** dopo essere stato riportato (non si nasconde niente), il ciclo dei fotogrammi viene **riavviato** — un fotogramma andato storto non uccide più la partita — e lo stesso errore ripetuto conta senza riempire lo schermo. `removeEventListener` continua a funzionare perché ogni funzione avvolta si porta dietro il riferimento all'originale.

Nello stesso giro è stato chiuso un errore vero della stessa famiglia: a fine sessione, se `cars` è vuoto — una qualifica in modalità spettatore, dove i giri dell'IA sono simulati e non guidati — «sono arrivati tutti» è vero a vuoto e il blocco del vincitore leggeva `sortedCars[0].driverName` di un elemento che non c'è. Su `file://` sarebbe apparso come l'ennesimo «Script error, riga 0».

**Tre note di igiene sulla traiettoria**, dalla stessa caccia:

- il **giudice non gira più in gioco**. Far scegliere la traiettoria da un giro simulato significa rientrare nella simulazione di gara da dentro `getRacingLine()`, che viene chiamata dalle schermate, dall'avvio di una sessione e dal libro dei record: secondi di blocco mentre qualcuno sta guardando, e una funzione pesante e rientrante in mezzo all'interfaccia. In gioco `RACING_LINE_JUDGE_REPS` è **0** e decide il proxy (succede solo per un circuito modificato dopo l'ultima generazione di `lines.js`, e costa 1-2 s una volta sola); `genlines.js` lo mette a 3, perché lì scegliere bene è tutto il mestiere;
...
### 2.4duodetricies-ter Spa, e cosa succede a ricalcare un circuito vero (Apex 3)

«Genera un tracciato che ricalchi Spa», con la mappa dei nomi allegata. Il circuito c'è, si chiama **Spa**, ed è il diciannovesimo: **3136 px** di mezzeria, il secondo più lungo del gioco dopo Kart, 56% del giro in curva, raggi da 75 a 148 px, carreggiata 50/70 come Kart e Anchor. Ci sono tutte e quindici le curve col loro nome, nell'ordine giusto e **ciascuna che gira dalla parte in cui gira in Belgio**: La Source, Eau Rouge, Raidillon, il rettilineo del Kemmel, Les Combes, Malmedy, la discesa al tornante di Bruxelles, Speaker's Corner, la Double Gauche (Pouhon), Les Fagnes, Campus, Stavelot, la Courbe Paul Frère, Blanchimont, il Bus Stop.

**È ruotato di 191 gradi rispetto a come lo si disegna di solito.** `checkLapCross()` conta il giro quando si attraversa `startX` andando in **+x**: il rettifilo del traguardo deve essere percorso da sinistra a destra, e sulla mappa va in basso a sinistra. Una rotazione è l'unica trasformazione che lascia il circuito *identico* — stesso ordine, stessi raggi, stesse lunghezze, e soprattutto stessa **mano** per ogni curva, quindi La Source resta una forcella a destra. Uno specchio sinistra-destra avrebbe accontentato lo stesso il contagiri e avrebbe reso Spa antioraria, che è un altro circuito. Quindi: box in alto a destra, Kemmel che scende verso sinistra, Stavelot in alto a sinistra.

**La regola che ha deciso tutto il resto, e che vale per qualunque circuito futuro.** La strada qui è larga 100 px su un'arena di 1150. Spa è lunga 7 km: un ricalco 1:1 mette Les Combes, il Bus Stop e il tornante di Rivage dentro uno spazio più stretto della carreggiata stessa — e una curva più stretta della propria strada non è una curva. La formula:

> il prato **dentro** una curva è un disco di raggio `R − trackWidth`. Se la curva gira più di ~80 gradi quel disco è l'unica cosa che sta fra le sue due gambe. Il muro esiste solo dove la distanza dalla mezzeria supera `wallRadius`, quindi serve `R > 68` perché ci sia una barriera e `R ≥ 86` perché sia larga 36 px. Sotto quella soglia l'apice è semplicemente asfalto: la curva si taglia dritta.

Le prime tre stesure sono morte tutte lì. Il controllo che le ha smascherate non è la distanza fra i punti della mezzeria — due punti vicini possono essere lo stesso pezzo di strada che curva, e va benissimo — ma questo: **camminando lungo la normale, quanto dura il prato prima che ricominci l'asfalto?** Sotto `2 × (wall − road) = 36 px` non c'è posto per i due muri e il prato non separa niente. Su Malmedy la prima stesura dava **32 px**, sul Bus Stop **12**.

**Quindi i complessi lenti sono più grandi del vero.** Ogni complesso che inverte il senso di marcia — La Source, Bruxelles, Pouhon, Campus/Stavelot — è disegnato come una U vera con `R ≈ 90` e le due gambe a 190 px. Quattro U in un'arena sola sono quasi tutto lo spazio, ed è il motivo per cui **il Kemmel è 318 px** invece del quarto di giro che è in Belgio. È il rettilineo più lungo del circuito (il secondo è 164) ma non del gioco, e questa è la concessione più grossa che il ricalco ha dovuto fare.

**La curva che non c'è è il gancio del Bus Stop.** La chicane vera sposta la strada di lato di circa 22 px a questa scala e ne servirebbero 136 perché ci stia dentro una barriera; fra Blanchimont e il rettifilo box ci sono 200 px e la salita dell'Eau Rouge sta esattamente sopra, quindi il gancio non ha dove andare. Quello che c'è è una finta a destra e una lunga sinistra sul rettifilo: il **ritmo** prima della linea sopravvive, lo stop-and-go no.

**Non disegnato: risolto.** Piazzare i vertici a mano e sperare nel raccordo non funziona, perché il vincolo non è dove sta il vertice ma quanto gira e con che raggio. Il circuito è quindi scritto come **lista di curve** — di quanto gira ciascuna, quanto dura la retta dopo, che raggio vuole — e un minimizzatore ai minimi quadrati (`spa2.py`) chiude il poligono (360 gradi di giro totale, spostamento zero) tenendo tre vincoli duri: 146 px fra strade non unite, l'arena, e il raggio minimo qui sopra. I pesi dicono cosa è sacro (le due metà della forcella della Source) e cosa può cedere. La repulsione fra i lati va accesa **dopo** la chiusura, in tre passate: accesa subito manda il minimizzatore in un groviglio, perché è un termine a soglia e da una configurazione qualsiasi tira più forte della chiusura.

**Misurato dopo.** Striscia di prato più stretta fra due passaggi **60 px** (Anchor, l'unico precedente con carreggiata uguale, sta a 64); distanza minima fra due strade non unite **146 px**; ingombro 1104 × 753 su 1150 × 765. Quattro gare dell'IA a `impossible`: dieci arrivati su dieci ogni volta, **zero ritiri, zero testacoda**, 0.24-0.77% del giro sull'erba, giro migliore 14.48-14.60 s (Kart 16.3, Anchor 12.9). La griglia — dieci auto a 30 px l'una dall'altra, 305 px all'indietro — sta tutta sull'asfalto e si arrotola intorno al Bus Stop, che è dove sta anche a Spa. Nella build con le soste un set di dure dura **3.35 giri** e uno di morbide 1.91: una gara da cinque giri è una sosta con la dura e due con la morbida, che è la forbice che serviva.

**Un difetto trovato per strada, e non è di Spa.** `genlines.js` **non è riproducibile**: rilanciato sulle stesse geometrie ha restituito una traiettoria diversa per **7 circuiti su 18** (il giudice fa girare qualifiche simulate, e quelle hanno del casuale dentro). L'impronta del libro dei record contiene la versione dell'ottimizzatore e la geometria, non la tabella: sette circuiti avrebbero cambiato i tempi con il libro che continuava a dichiararsi valido. Quindi la tabella spedita è quella **di prima**, con la sola voce di Spa aggiunta in coda. Regola nuova: quando si aggiunge un circuito si aggiunge **una voce**, non si rigenera il file.
...
- **[Apex 3] Quando cambia un circuito, l'ottimizzatore della traiettoria o la fisica che rispecchia, rilancia `genlines.js` (e alza `RACING_LINE_VERSION` se è cambiato il metodo).** `lines.js` è la tabella delle traiettorie per impronta geometrica: un circuito che non trova la sua voce si ottimizza a caldo — giusto, ma 1-3 s la prima volta, e a catena su tutti i diciotto se manca il file. La versione sta anche nell'impronta del libro dei record: cambiarla lo ricostruisce.
- **[Apex 3] "Al limite della macchina" si dice di una linea, non di un'auto.** Cinque manopole scandite a vuoto non provano che non ci sia più passo: provano che quelle manopole non sono il collo di bottiglia. La traccia di velocità su un circuito dove una persona va il 14% più forte ha trovato in mezz'ora quello che un anno di scansioni non aveva trovato. Prima di dichiarare un tetto, guardare *dove* l'IA rallenta e chiedersi se deve.
- **[Apex 3] Una linea vera toglie il margine che una linea brutta regalava per caso.** Il profilo di velocità dell'IA era della linea e non della macchina: finché la linea stava all'interno, stare 30 px larghi era gratis; appena va al bordo, è erba e poi muro. Ogni volta che si migliora la *posizione* dell'IA bisogna guardare cosa ci si appoggiava sopra senza saperlo.
- **[Apex 3] Un proxy va tarato su una traccia, non solo sul tempo finale.** Il primo modello della traiettoria mancava la resistenza da deriva e la S di Thunder: sommava bene (7.1 s contro 8.3 reali) e ordinava male. Confrontare nodo per nodo la velocità prevista e quella guidata ha mostrato dove mentiva (l'uscita di curva, −15%) e perché. E comunque la scelta finale la fa la simulazione vera, non il proxy.
- **[Apex 3] Su gare a dieci vetture i ritiri si contano a decine di gare, non a tre.** 36 gare danno ±6 ritiri di rumore: sei configurazioni "diverse" sono risultate indistinguibili. I **contatti pesanti** (≥40 danni, 3 a gara) danno una statistica quattro volte migliore con lo stesso costo; e prima di ottimizzare un numero, misurare qual è quello del gioco vero (nei log di Nicola: 1.23 ritiri IA a gara).
- **[Apex 3] Se un `return` all'inizio del ciclo mette in pausa il mondo, il codice che toglie quella pausa non può stare dentro il ciclo.** Il pit wall aperto fermava `gameLoop` prima di `updatePhysics`, ed era `updatePhysics` a contenere la logica che lo richiudeva: uno stallo che nessuna partita successiva poteva sciogliere. Ogni "hold" va scritto con la sua condizione di uscita **nello stesso punto**: se non c'è più una gara o una macchina a cui appartiene, si stand down lì.
...
- **[Apex 3] Una curva piu' stretta della propria carreggiata non e' una curva.** Il prato dentro una curva e' un disco di raggio `R - trackWidth`, e oltre gli ~80 gradi di rotazione e' l'unica cosa fra le due gambe: sotto `R = wallRadius` non c'e' barriera e l'apice si taglia dritto. E' il vincolo che ha ucciso tre stesure di Spa prima che lo scrivessi. Il test che lo trova non e' la distanza fra punti della mezzeria - due punti vicini possono essere lo stesso pezzo di strada - ma **quanto dura il prato lungo la normale prima che ricominci l'asfalto**.
- **[Apex 3] Ricalcare un circuito vero significa scegliere cosa sacrificare, e dirlo.** A 0.45 px/metro le curve lente di Spa stanno dentro uno spazio piu' piccolo della strada del gioco: o si ingrandiscono fuori scala, o spariscono. Ingrandire le quattro U (Source, Bruxelles, Pouhon, Campus/Stavelot) e' costato il rettilineo del Kemmel, che e' 318 px invece del quarto di giro che e' in Belgio. La scelta e' difendibile; nasconderla no.
