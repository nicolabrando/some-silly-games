...
### 2.4duodetricies-ter Spa: costruita, misurata, tolta (Apex 3)

«Genera un tracciato che ricalchi Spa», e poi, guardandolo: «non mi piace Spa, toglila». Il circuito è stato rimosso — non c'è più nella build, e `lines.js` è tornato bit a bit alla tabella di prima. Resta qui perché una cosa l'ha insegnata, e vale per qualunque circuito futuro:

> il prato **dentro** una curva è un disco di raggio `R − trackWidth`. Se la curva gira più di ~80 gradi quel disco è l'unica cosa fra le sue due gambe. Il muro esiste solo dove la distanza dalla mezzeria supera `wallRadius`, quindi serve `R > 68` perché ci sia una barriera e `R ≥ 86` perché sia larga 36 px. Sotto quella soglia l'apice è asfalto e la curva si taglia dritta.

Il controllo che lo trova non è la distanza fra punti della mezzeria — due punti vicini possono essere lo stesso pezzo di strada che curva, e va benissimo — ma questo: **camminando lungo la normale, quanto dura il prato prima che ricominci l'asfalto?** Sotto `2 × (wall − road) = 36 px` non c'è posto per i due muri. Sulle prime stesure di Spa quel numero valeva 12 px al Bus Stop e 32 a Malmedy, e il gioco disegnava una chicane che si poteva attraversare in linea retta.

L'altra cosa imparata è di metodo: **un tracciato si risolve, non si disegna**. Piazzare i vertici a mano e lasciare che il raccordo si arrangi schiaccia i raggi proprio dove servono grandi. Scriverlo come lista di curve (di quanto gira, quanto dura la retta dopo, che raggio vuole) e chiudere il poligono con un minimizzatore ai minimi quadrati — 360 gradi di giro totale, spostamento zero, con arena e distanze come vincoli — converge in secondi. Un termine a soglia come la repulsione fra i lati va acceso **dopo** la chiusura e per gradi: acceso subito tira più forte di lei e il poligono si annoda. Gli attrezzi sono in `spa2.py`, e servono al prossimo circuito qualunque esso sia.

E un difetto trovato per strada, che con Spa non c'entra: **`genlines.js` non è riproducibile.** Rilanciato sulle stesse geometrie ha restituito una traiettoria diversa per **7 circuiti su 18** — il giudice fa girare qualifiche simulate, e quelle hanno del casuale dentro. L'impronta del libro dei record contiene la versione dell'ottimizzatore e la geometria, non la tabella: sette circuiti avrebbero cambiato i tempi con il libro che continuava a dichiararsi valido. Regola: quando si aggiunge un circuito si aggiunge **una voce** alla tabella, non si rigenera il file.

### 2.4duodetricies-quater L'acqua ferma non deve poter fermare una rotazione (Apex 3)

«Perché con le ultime modifiche alle Drift non si riesce più a driftare nelle pozzanghere? Dovrebbe essere il posto più facile per farlo.»

Aveva ragione, ed era colpa del tetto d'imbardata di §2.4duodetricies. Misurato a gas e sterzo piantati per 1.5 s, gomma Drift:

| | tetto | coda chiesta → data | deriva dopo 1.5 s |
|---|---|---|---|
| asciutto | 17.6 | 1.73 → 1.73 | 53° |
| bagnato (strada) | 1.61 | 1.25 → 0.81 | 25° |
| **pozzanghera** | **0.64** | **1.51 → 0.15** | **6-12°** |

In pozzanghera l'aderenza viene tagliata due volte — prima dalla pioggia, poi dall'acqua ferma (`(0.45 + 0.30·aqua) − 0.25·aquaplane`, cioè fino a un quinto) — e il tetto, che è `YAW_CAP · currentGrip / v`, crollava con lei. Del tetto lo sterzo si prendeva già 0.49: alla coda restavano **quindici centesimi** di quello che il motore chiedeva. Il posto dove una macchina scambia le estremità più facilmente era diventato l'unico posto dove non poteva ruotare affatto. Peggio: la **full wet**, che l'acqua la pompa via e quindi conserva aderenza, in pozzanghera derapava (39°) mentre la Drift no. Una gomma da pioggia che deraperà meglio di una gomma da drift è la firma di un modello sbagliato.

**Dove sbagliava il ragionamento.** Il tetto viene da: un'auto al limite in curva ha `a = v²/R ≤ currentGrip` e ruota a `ω = a/v`, quindi la strada non può pagare più di `currentGrip/v` di rotazione. È giusto per la parte di rotazione che la strada deve **creare**. Ma l'attrito è anche quello che una rotazione la **ferma**, e sull'acqua ferma non c'è né l'uno né l'altro. Un tetto proporzionale all'aderenza tiene conto solo della prima metà, e conclude che sul ghiaccio un'auto non può girarsi — che è l'esatto contrario di quello che fa il ghiaccio.

**La correzione.** Il tetto si misura sull'aderenza della **strada**, presa prima dell'acqua ferma; il fattore della pozzanghera può abbassarlo solo fino a un pavimento, `YAW_WATER_FLOOR`. Tutto il resto della pozzanghera è intatto: l'aderenza laterale crolla lo stesso, l'aquaplaning uccide lo sterzo lo stesso (`aqua` moltiplica solo la parte anteriore, ed è scritto così da sempre), la resistenza che frena l'auto è la stessa. Cambia solo che l'acqua non può più impedire alla coda di uscire.

**La taratura, che è tutta nel pavimento.** Non è gratis: se l'acqua non abbassa affatto il tetto, in pozzanghera si girano anche le slick, e l'IA ne paga il conto. Dodici gare bagnate a `impossible` su quattro circuiti, per ogni valore:

| pavimento | ritiri | fotogrammi girati | erba | danni | deriva Drift in pozza |
|---|---|---|---|---|---|
| 0 (com'era) | 16 / 19 | 235 / 231 | 1.32% / 1.58% | 9585 / 11181 | 7-8° |
| 0.75 | 19 | 192 | 1.58% | 10035 | 26-29° |
| **0.85** | **19** | **249** | **1.86%** | **12020** | **33-34°** |
| 1 (libero) | 25 | 6115 | 2.68% | 11915 | 41-44° |

La riga «com'era» è misurata **due volte** apposta: la banda di rumore su dodici gare è 16-19 ritiri e 9.6-11.2k di danni, e serve saperla prima di dichiarare che qualcosa è cambiato. A **0.85** ritiri e testacoda dell'IA stanno dentro quella banda; a 1 no, e il salto fra 0.85 e 1 è un muro, non una china. Il giocatore intanto passa da 7-8° a **33-34°** di deriva in pozzanghera contro i 24-26° che ha sulla strada bagnata: la pozzanghera torna a essere il posto più facile dove metterla di traverso, che era la richiesta.
