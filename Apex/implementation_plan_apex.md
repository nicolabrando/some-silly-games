...
**Il controllo che ha salvato Thunder.** Allungare in verticale può *avvicinare* due tratti di strada che non si toccano lungo il giro. Su Thunder, a `sy = 1.30`, la punta in alto si è pizzicata: la distanza minima fra due tratti non adiacenti è scesa da 168px a 124px, sotto i 140 che servono (`trackWidth + grassWidth`) perché fra le due strade resti una barriera. Le strade non si sovrapponevano ancora, ma `getClosestPoint` avrebbe potuto agganciare una vettura al tratto sbagliato — la stessa classe di bug del contagiri che salta un giro. Thunder è quindi rimasto a `sy = 1.00`: distanza 147px, e per inciso il tempo sul giro torna identico a prima (14.87 contro 14.79), mentre a 1.30 era salito a 16.23. `clearance.js` misura questa distanza su tutti gli 11.
